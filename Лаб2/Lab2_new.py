...
for i in range(100):
    ...
    def matrix(matr):
        return (matr[0][0] * matr[1][1] * matr[2][2] + matr[0][1] * matr[1][2] * matr[2][0] + matr[1][0] * matr[2][1] *
                matr[0][2]) - \
               (matr[0][2] * matr[1][1] * matr[2][0] + matr[0][1] * matr[1][0] * matr[2][2] + matr[0][0] * matr[1][2] *
                matr[2][1])
    ...
    near_value = 20
    for i in Rkr:
        if abs(i - m) < abs(near_value - m):
            near_value = i

    if False not in [i < Rkr[near_value][t] for i in ruv]:
        odn += 1
    else:
        ne_odn += 1

print("Кількість однорідних дисперсій -", odn, ",\nКількість неоднорідних дисперсій -", ne_odn)
